...
CREATE TABLE IF NOT EXISTS `movierating`.`director` (
  `iddirector` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'ID is used as a Primary key of the director table.',
...
CREATE TABLE IF NOT EXISTS `movierating`.`film` (
  `idfilm` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'ID is used as a Primary key of the film table.',
  ...
  `iddirector` INT(10) UNSIGNED NOT NULL COMMENT 'Directors id to connect with a director table.',
  ...
  INDEX `director_name_idx` (`iddirector` ASC, `name` ASC)  COMMENT 'Index to find film (TV series) by its name and director or just by its director, because director can be used as independently indexed.',
  CONSTRAINT `iddirector_film_fk`
    FOREIGN KEY (`iddirector`)
    REFERENCES `movierating`.`director` (`iddirector`)
    ON UPDATE CASCADE)
ENGINE = InnoDB
AUTO_INCREMENT = 11
DEFAULT CHARACTER SET = utf8
...
CREATE TABLE IF NOT EXISTS `movierating`.`film_genre` (
  ...
  PRIMARY KEY (`idfilm`, `genrename`)  COMMENT 'Primary key of the table.',
...
CREATE TABLE IF NOT EXISTS `movierating`.`user` (
  `iduser` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'ID is used as a Primary key of the user table.',
...
CREATE TABLE IF NOT EXISTS `movierating`.`rating` (
  `idrating` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'ID is used as a Primary key of the ratin table.',
  `iduser` INT(10) UNSIGNED NOT NULL COMMENT 'User ID, which is used as a FK to connect with a user table.',
  `idfilm` INT(10) UNSIGNED NOT NULL COMMENT 'User ID, which is used as a FK to connect with a film table.',
  ...
  INDEX `iduser_idx` (`iduser` ASC)  COMMENT 'Auto-created index for FK on user table.',
  INDEX `idfilm_idx` (`idfilm` ASC)  COMMENT 'Auto-created index for FK on film table.',
  INDEX `rating_idfilm_idx` (`rating_amount` ASC, `idfilm` ASC)  COMMENT 'Index can be used as an independent one for rating field and also to find films with a selected rating. In a such way users rating can be automatically increased or decreased.',
...
CREATE TABLE IF NOT EXISTS `movierating`.`review` (
  `idreview` INT(11) NOT NULL AUTO_INCREMENT COMMENT 'ID is used as a Primary key of the review table.',
  `iduser` INT(10) UNSIGNED NOT NULL COMMENT 'User ID, which is used as a FK to connect with a user table.',
  `idfilm` INT(10) UNSIGNED NOT NULL COMMENT 'User ID, which is used as a FK to connect with a film table.',
  ...
  INDEX `film_user_idx` (`idfilm` ASC, `iduser` ASC)  COMMENT 'Index, that helps to find review to the selected film by some user.',
  INDEX `user_date_idx` (`iduser` ASC, `date` ASC)  COMMENT 'Index, which helps to find review of the selected user on a certain date. Also it can be used as an index to find review by certain users.',
